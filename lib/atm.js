...
module.exports = function(){
    ...
    self.create = create;
    self.checkBalance = (id,pin,result) => { database.read(id,pin,result); };
    self.deposit  = deposit ;
    self.withdraw = withdraw;
    self.clear = (cb) => { self.currency = "USD"; self.amount = 0; self.database.clear(cb); }

    function create(id,pin,callback){
        database.create(id,pin,callback);
    }
    ...
    function update(id,pin, amount, callback) {
        ...
        database.read(id, pin, function (result, currentAmount){
            if(result == 1){
                ...
                callback(result, "There was a problem retrieving the account. " + currentAmount);
            }
        });
    }
